...
var v0 = 0.19511;
var r = 140;
//var difference = 0.00012893;
//var difference = 0.0001289283;
var difference = 0.00012882;
var index;
var number = 22;
...
function Start() {
    ...
    index = (getRotateAngle($('.roulette-wheel')[0]) % 360) * 37 / 360;
    console.log(getRotateAngle($('.roulette-wheel')[0]) % 360 + " " + index);
    if (index >= 4 && index <= 13) difference -= 0.00000185;
    if (index >= 28 && index <= 32) difference += 0.00000185;
    if (index > 320) difference += 0.00000205;
   // if (index >= 15 && index <= 25) difference += 0.00000185;
   // if (index > 25 && index <= 32) difference -= 0.00000185;
    v = v0 + difference * (37 - index) - 0; //ROULETTE_SPEED * 0.000077;
    r = 140;
    $("#ball").move();
}

function getRotateAngle(el) {
    var degree = el.style[TRANSFORM];
    var regexPattern = /rotate\((\d+\.?\d?)deg\)/g;
    var match = regexPattern.exec(degree);
    return match[1];
}
